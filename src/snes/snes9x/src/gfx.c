...
void S9xRenderLine_NoFramebuffer(uint32_t line, S9xLineCallback cb)
{
    if (!cb)
        return;
    ...
    /* Save old state */
    bool     oldLineMode   = GFX.LineRenderMode;
    uint32_t oldPrevLine   = IPPU.PreviousLine;
    uint32_t oldCurrLine   = IPPU.CurrentLine;
    ...
    /* We want the core to render 1 line */
    ...
    /* 
     ...
     We just need to point GFX.Screen, GFX.SubScreen, GFX.ZBuffer and GFX.SubZBuffer
     to the right line inside these buffers.
    */
    uint8_t *newScreenBase    = (uint8_t *)s_line.main - line * GFX.Pitch2;
    uint8_t *newSubScreenBase = (uint8_t *)s_line.sub  - line * GFX.Pitch2;
    uint8_t *newZBase         = (uint8_t *)s_line.z    - line * GFX.ZPitch;
    uint8_t *newSubZBase      = (uint8_t *)s_line.subz - line * GFX.ZPitch;

    GFX.Screen     = newScreenBase;
    GFX.SubScreen  = newSubScreenBase;
    GFX.ZBuffer    = newZBase;
    GFX.SubZBuffer = newSubZBase;

    // Delta / DepthDelta = distance between Screen/SubScreen and Z/SubZ
    ...
    IPPU.PreviousLine    = oldPrevLine;
    IPPU.CurrentLine     = oldCurrLine;
