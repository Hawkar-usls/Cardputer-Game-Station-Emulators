...
static uint32_t ssg_mask_any = 0; // 1 bit par slot
...
unsigned int* sin_tab = NULL;   // SIN_LEN;
...
#if GW_TARGET
static UINT8* lfo_pm_table = NULL;  /* 128 combinations of 7 bits meaningful (of F-NUMBER), 8 LFO depths, 32 LFO output levels per one depth */
#else
...
INLINE void IRAM_ATTR FM_KEYOFF_CSM(FM_CH *CH , int s )
{
  FM_SLOT *SLOT = &CH->SLOT[s];
  if (!SLOT->key)
  {
...
INLINE void IRAM_ATTR update_ssg_eg_channels_masked(FM_CH *CH, uint32_t mask /* 24 bits, 4 bits/chan */)
{
  ...
  {
    if ((mask & 0xF) == 0) continue; // aucun slot SSG sur ce canal
    // on traite seulement les slots dont le bit3 (0x8) est actif
    for (int s = 0; s < 4; s++)
    {
      if (((mask >> s) & 1) == 0) continue;
      FM_SLOT *SLOT = &CH->SLOT[s];

      if ((SLOT->volume >= 0x200) && (SLOT->state > EG_REL))
      {
        if (SLOT->ssg & 0x01) {
          ...
          if (SLOT->ssg & 0x02) SLOT->ssgn ^= 4; else SLOT->phase = 0;
...
INLINE void IRAM_ATTR chan_calc(FM_CH *CH, int num)
{
  do
  {
    /* AM pré-calculé pour le canal */
    const UINT32 AM_now = ym2612.OPN.LFO_AM >> CH->ams;

    /* Pointeurs rapides sur les slots */
    FM_SLOT *const S1 = &CH->SLOT[SLOT1];
    FM_SLOT *const S2 = &CH->SLOT[SLOT2];
    FM_SLOT *const S3 = &CH->SLOT[SLOT3];
    FM_SLOT *const S4 = &CH->SLOT[SLOT4];

    /* EG des 4 slots (avec AM si activé) */
    unsigned eg1 = S1->vol_out + (S1->AMmask ? AM_now : 0);
    unsigned eg2 = S2->vol_out + (S2->AMmask ? AM_now : 0);
    unsigned eg3 = S3->vol_out + (S3->AMmask ? AM_now : 0);
    unsigned eg4 = S4->vol_out + (S4->AMmask ? AM_now : 0);

    /* clear des bus internes */
    m2 = c1 = c2 = mem = 0;

    /* restaurer MEM (delayed) */
    *CH->mem_connect = CH->mem_value;

    /* ----- Feedback OP1 (identique, mais compact) ----- */
    INT32 fb_acc = CH->op1_out[0] + CH->op1_out[1];
    CH->op1_out[0] = CH->op1_out[1];

    if (!CH->connect1) {
      /* algo 5 : connect1==NULL → copie sur mem/c1/c2 */
    ...
    if (eg1 < ENV_QUIET) {
      if (!CH->FB) fb_acc = 0;
      CH->op1_out[1] = op_calc1(S1->phase, eg1, (fb_acc << CH->FB));
    }

    /* ----- Slots 3/2/4 : court-circuit si tous muets ----- */
    const int any_active_234 = (eg2 < ENV_QUIET) | (eg3 < ENV_QUIET) | (eg4 < ENV_QUIET);
    if (any_active_234) {
      /* SLOT3 */
      if (eg3 < ENV_QUIET)
        *CH->connect3 += op_calc(S3->phase, eg3, m2);

      /* SLOT2 */
      if (eg2 < ENV_QUIET)
        *CH->connect2 += op_calc(S2->phase, eg2, c1);

      /* SLOT4 */
      if (eg4 < ENV_QUIET)
        *CH->connect4 += op_calc(S4->phase, eg4, c2);
    }

    /* mémorise MEM courant */
    CH->mem_value = mem;

    /* ----- Avance de phase (inchangée) ----- */
    if (CH->pms) {
      if ((ym2612.OPN.ST.mode & 0xC0) && (CH == &ym2612.CH[2])) {
        update_phase_lfo_slot(S1, CH->pms, ym2612.OPN.SL3.block_fnum[1]);
        update_phase_lfo_slot(S2, CH->pms, ym2612.OPN.SL3.block_fnum[2]);
        update_phase_lfo_slot(S3, CH->pms, ym2612.OPN.SL3.block_fnum[0]);
        update_phase_lfo_slot(S4, CH->pms, CH->block_fnum);
      } else {
...
INLINE void OPNWriteReg(int r, int v)
{
  ...
  switch( r & 0xf0 ) {
    ...
    case 0x90:  /* SSG-EG */
    {
      const int slot_lin = c*4 + OPN_SLOT(r);

      SLOT->ssg  = v & 0x0f;

      if (v & 0x08)  ssg_mask_any |=  (1u << slot_lin);
      else           ssg_mask_any &= ~(1u << slot_lin);

      /* recalc EG out (inchangé) */
      if (SLOT->state > EG_REL)
      {
        if ((SLOT->ssg&0x08) && (SLOT->ssgn ^ (SLOT->ssg&0x04)))
          SLOT->vol_out = ((UINT32)(0x200 - SLOT->volume) & MAX_ATT_INDEX) + SLOT->tl;
...
static inline void YM2612Update(int16_t *buffer, int length)
{
  ...
  /* ==== Hoist dacen/mode invariants sur toute la frame ==== */
  const int dacen_now = ym2612.dacen;
  const int mode_now  = ym2612.OPN.ST.mode & 0xC0;

  /* === refresh FC/EG requis (hors boucle) === */
  ...
  {
    /* 3SLOT MODE (operator order 0,1,3,2) */
  ...
  for (i = 0; i < length; i++)
  {
    ...
    /* update SSG-EG */
    if (ssg_mask_any) {
      update_ssg_eg_channels_masked(&ym2612.CH[0], ssg_mask_any);
    }
    ...
    /* clamp 14-bit par canal */
#if defined(__ARM_NEON) || defined(__ARM_NEON__)
    {
      int32x4_t v0123 = vld1q_s32((const int32_t*)&out_fm[0]);
      int32x2_t v45   = vld1_s32 ((const int32_t*)&out_fm[4]);
      const int32x4_t vmax14 = vdupq_n_s32( 8191);
      const int32x4_t vmin14 = vdupq_n_s32(-8192);

      v0123 = vminq_s32(v0123, vmax14);
      v0123 = vmaxq_s32(v0123, vmin14);

      v45 = vmin_s32(v45, vget_low_s32(vmax14));
      v45 = vmax_s32(v45, vget_low_s32(vmin14));

      /* somme horizontale */
      int32x2_t sum01 = vadd_s32(vget_low_s32(v0123), vget_high_s32(v0123)); // [0+1, 2+3]
      int32x2_t s0123 = vpadd_s32(sum01, sum01);                              // [0+1+2+3, ...]
      int32x2_t s45   = vpadd_s32(v45, v45);                                  // [4+5, ...]
      int32x2_t all   = vadd_s32(s0123, s45);
      *buffer++ = (int16_t)vget_lane_s32(all, 0);
    }
#else
    {
      int a0 = clamp14(out_fm[0]);
      int a1 = clamp14(out_fm[1]);
      int a2 = clamp14(out_fm[2]);
      int a3 = clamp14(out_fm[3]);
      int a4 = clamp14(out_fm[4]);
      int a5 = clamp14(out_fm[5]);
      int mix = a0 + a1 + a2 + a3 + a4 + a5;
      *buffer++ = (int16_t)mix;
    }
#endif
  ...
  /* timer B control en fin de frame */
...
void gwenesis_ym2612_tables_alloc(void) {
    if (!tl_tab) {
        tl_tab = (signed int*) calloc(TL_TAB_LEN, sizeof(signed int));
    }
    if (!sin_tab) {
