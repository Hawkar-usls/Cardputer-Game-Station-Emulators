...
void RunPCE(void)
{
    ...
    while (running)
    {
        ...
        uint64_t frameTime = now - lastFrameEnd;
        lastFrameEnd       = now;
        bool skipThisFrame = skipNextFrame;
        skipNextFrame      = false;
        ...
        // Frameskip adaptatif, if we clearly exceed the frame budget
        if (frameTime > frameDurationUs + (frameDurationUs / 5)) {
            // > 1.2x time for the frame
            skipNextFrame = true;
        }
        ...
        if (now - lastFpsTime >= 1000000ULL) {
            ...
            printf("[PCE] FPS: %.2f | HEAP: %u | skipNext=%d\n",
                   fps, esp_get_free_heap_size(), (int)skipNextFrame);
